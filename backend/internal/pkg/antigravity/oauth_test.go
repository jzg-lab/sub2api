...
func TestGetClientSecret_环境变量为空(t *testing.T) {
	...
	if err == nil {
		t.Fatal("client_secret 环境变量为空时应返回错误")
	}
...
func TestGetClientSecret_环境变量未设置(t *testing.T) {
	t.Setenv(AntigravityOAuthClientIDEnv, "test-antigravity-client")
	t.Setenv(AntigravityOAuthClientSecretEnv, "")
	if err := os.Unsetenv(AntigravityOAuthClientSecretEnv); err != nil {
		t.Fatal(err)
	}

	_, err := getClientSecret()
	if err == nil {
		t.Fatal("client_secret 环境变量未设置时应返回错误")
	}
}
...
func TestGetClientSecret_环境变量含空格(t *testing.T) {
	...
	_, err := getClientSecret()
	if err == nil {
		t.Fatal("client_secret 环境变量仅含空格时应返回错误")
	}
}
...
func TestBuildAuthorizationURL_参数验证(t *testing.T) {
	...
	expectedParams := map[string]string{
		"client_id":              "test-antigravity-client",
