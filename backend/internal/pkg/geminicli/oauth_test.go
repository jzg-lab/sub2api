...
func TestBuildAuthorizationURL(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientIDEnv, "test-gemini-cli-client")
...
func TestBuildAuthorizationURL_MissingEnvSecret(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) || authURL != "" {
		t.Fatalf("expected missing-secret error and no URL, got %q, %v", authURL, err)
	}
}
