...
//
// If ClientID/ClientSecret is not provided, use the privately configured CLI client.
//
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// Both defaults are private runtime configuration, without embedded fallbacks.
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		return OAuthConfig{}, infraerrors.New(http.StatusBadRequest, "GEMINI_OAUTH_CLIENT_NOT_CONFIGURED", "OAuth client not configured: please set both client_id and client_secret (or leave both empty to use the privately configured Gemini CLI client)")
	}

	isBuiltinClient := effective.ClientID == GetOAuthClientID()
