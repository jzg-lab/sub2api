...
func (s *GeminiOAuthService) GetOAuthConfig() *GeminiOAuthCapabilities {
	...
	enabled := clientID != "" && clientSecret != "" && clientID != geminicli.GetOAuthClientID()
...
func (s *GeminiOAuthService) GenerateAuthURL(ctx context.Context, proxyID *int64, redirectURI, projectID, oauthType, tierID string) (*GeminiAuthURLResult, error) {
	...
	// - code_assist: use the CLI client provided through private environment configuration
	// - google_one: use the same privately configured CLI client
	...
	isBuiltinClient := effectiveCfg.ClientID == geminicli.GetOAuthClientID()
