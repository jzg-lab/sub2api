...
> For the jzg-lab custom distribution, read
> [repository maintenance](../docs/REPOSITORY_MAINTENANCE.md) first.
> Use `compose.relay.yml` to build this source. The upstream images and installer
> examples below do not deploy the custom version. Antigravity and Gemini CLI
> OAuth require private client ID/secret configuration before login or refresh.
...
**Private configuration required** - uses the operator-provided CLI OAuth client.

1. Set `GEMINI_CLI_OAUTH_CLIENT_ID` and `GEMINI_CLI_OAUTH_CLIENT_SECRET` privately.
   The separate `GEMINI_OAUTH_CLIENT_ID` / `GEMINI_OAUTH_CLIENT_SECRET` settings are for AI Studio.
...
# 可选：如需使用 Gemini CLI OAuth Client（Code Assist / Google One）
# 安全说明：本仓库不内置客户端凭据，请在私有运行环境中同时配置 ID 和 Secret。
# GEMINI_CLI_OAUTH_CLIENT_ID=your-cli-client-id
...
2. Add your OAuth credentials in the `[Service]` section (after the existing `Environment=` lines):
   ...
   ```ini
   Environment=GEMINI_CLI_OAUTH_CLIENT_ID=your-cli-client-id
