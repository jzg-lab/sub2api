<template>
  ...
  >
    ...
    >
      ...
      <!-- OpenAI basispoints 出站模式（OAuth / Setup Token） -->
      <div
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.basispointsMode') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.basispointsModeDesc') }}
            </p>
          </div>
          <button
            type="button"
            data-testid="edit-openai-basispoints-mode-toggle"
            @click="openaiBasispointsModeEnabled = !openaiBasispointsModeEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              openaiBasispointsModeEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                openaiBasispointsModeEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
const openaiBasispointsModeEnabled = ref(false)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  openaiBasispointsModeEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    openaiBasispointsModeEnabled.value =
      (newAccount.type === 'oauth' || newAccount.type === 'setup-token') && extra?.openai_basispoints_mode === true
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      // 缺省即走标准 codex 端点，关闭时删除字段而不是写 false
      if ((props.account.type === 'oauth' || props.account.type === 'setup-token') && openaiBasispointsModeEnabled.value) {
        newExtra.openai_basispoints_mode = true
      } else {
        delete newExtra.openai_basispoints_mode
      }
